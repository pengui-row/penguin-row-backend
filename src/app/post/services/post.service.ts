...
export class PostService {
  ...
  async getPosts(
    ...
    const queryBuilder = this.postRepository
      ...
      .loadRelationCountAndMap(
        ...
        'post.comments',
        'comment',
        (qb) => qb.andWhere('comment.status = :status', { status: 'ACTIVE' }),
      );
    queryBuilder
    ...
    if (user) {
      ...
      if (postIds.length > 0) {
        ...
        userFavoritePostIds = new Set(
          userFavorites.map((favorite) => favorite.postId),
        );
      }
    }
    ...
    try {
      ...
      if (!updateLike) {
        throw new NotFoundException(
          `Like no encontrado para el usuario ${user.id} y el post ${post}`,
        );
      }
  ...
  async unfavoritePost(post: string, user: User) {
    try {
      ...
      if (!updatedFavorite) {
        throw new NotFoundException(
          `Favorito no encontrado para el usuario ${user.id} y el post ${post}`,
        );
      }
  ...
  async getComments(postId: string, page: number = 1, page_size: number = 10) {
    try {
      ...
      const [comments, total] = await this.commentRepository
        ...
        .andWhere('comment.postId = :post', { post: postId })
    ...
    try {
      if (!user || !user.id) {
        throw new InternalServerErrorException(
          'El usuario es requerido para obtener posts favoritos.',
        );
      }
      ...
      const queryBuilder = this.postRepository
        ...
        .innerJoin(
          'post.favorites',
          'favorite',
          'favorite.userId = :userId AND favorite.status = :status',
          {
            userId: user.id,
            status: Status.ACTIVE,
          },
        ...
        .loadRelationCountAndMap(
          ...
          'like',
          (qb) =>
            qb.andWhere('like.status = :status', { status: Status.ACTIVE }),
        )
        .loadRelationCountAndMap(
          'post.commentsCount',
          'post.comments',
          'comment',
          (qb) =>
            qb.andWhere('comment.status = :status', { status: Status.ACTIVE }),
        );
      ...
      if (!posts || posts.length === 0) {
        throw new NotFoundException(
          'No se encontraron publicaciones favoritas para este usuario.',
        );
      }
  ...
  async getUserPost(page: number = 1, page_size: number = 10, user: User) {
    try {
      if (!user || !user.id) {
        throw new InternalServerErrorException(
          'El usuario es requerido para obtener sus posts.',
        );
      }
      ...
      if (!posts || posts.length === 0) {
        throw new NotFoundException(
          'No se encontraron publicaciones creadas por este usuario.',
        );
      }
  ...
  async getSearchedPost(
    ...
    try {
      ...
      const queryBuilder = this.postRepository
        ...
        .leftJoinAndSelect('user.profile', 'profile')
        .leftJoinAndSelect('post.tags', 'tag')
        .where('post.content LIKE :searchContent', {
      ...
      if (!posts || posts.length === 0) {
        throw new NotFoundException(
          `No se encontraron publicaciones que contengan "${search.search}".`,
        );
      }
